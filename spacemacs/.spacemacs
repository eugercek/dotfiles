...
values."
  (setq-default
   ...
   '(vimscript(custom-set-variables
...
(custom-set-faces
 ...
 '(bold ((t (:foreground "MediumOrchid2" :weight bold :height 1.1))))
...
`dotspacemacs/user-config' first."
  ...
  (setq-default evil-snipe-enable-incremental-highligh t)
...
(defun dotspacemacs/user-config ()
  ...
  (defun run-python-other-window ()
    ...
    (comint-send-string "*ansi-term*" "python main.py\n"))

  (spacemacs/set-leader-keys "op" 'run-python-other-window)

  (defun my_fill_region ()
    (interactive)
    (if (use-region-p)
        (fill-region (region-beginning) (region-end))
      (org-fill-paragraph)))
  (spacemacs/set-leader-keys "of" 'my_fill_region)
